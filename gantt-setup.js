...
const OWNER_OPTIONS = ["堀尾", "黒見", "大西(元)", "大西(優)", "木本", "前田", "本郷", "大重", "未定"];
...
const _ganttEndDate = new Date(); _ganttEndDate.setFullYear(_ganttEndDate.getFullYear() + 2); _ganttEndDate.setMonth(_ganttEndDate.getMonth() + 1, 1);
gantt.config.end_date = _ganttEndDate;            // 今日から2年先の月末まで
    ...
    function renderField(def) {
        if (def.inputType === "owner_multi") {
            const chks = OWNER_OPTIONS.map(function(n) {
                return `<label style="display:inline-flex;flex-direction:row;align-items:center;gap:4px;height:26px;padding:0 8px;cursor:pointer;white-space:nowrap;font-size:12px;box-sizing:border-box;border:1px solid #ccc;border-radius:4px;background:#fff;"><input type="checkbox" class="owner-me-chk" value="${n}" style="width:13px;height:13px;margin:0;"> ${n}</label>`;
            }).join('');
            return `<label class="me-owner-label">${def.label}<div style="display:flex;flex-wrap:wrap;gap:6px;" data-me-owner-wrap>
                ${chks}
...
gantt.form_blocks["trip_pn_lb"] = {
    render: function(sns) {
        return "<div class='gantt_cal_ltext'><input type='text' id='trip_lb_pn' style='width:100%;height:28px;border:1px solid #ccc;border-radius:4px;padding:0 6px;font-size:13px;box-sizing:border-box;' oninput='_tripLbAutoFill(this.value)'></div>";
    },
...
gantt.form_blocks["trip_text_lb"] = {
    render: function(sns) {
        return "<div class='gantt_cal_ltext'><input type='text' id='trip_lb_" + sns.name + "' style='width:100%;height:28px;border:1px solid #ccc;border-radius:4px;padding:0 6px;font-size:13px;box-sizing:border-box;'></div>";
    },
    set_value: function(node, value, task, sns) {
        node.querySelector("input").value = value || '';
    },
...
gantt.form_blocks["wish_date_lb"] = {
    render: function(sns) {
        return "<div class='gantt_cal_ltext'><input type='date' style='width:130px;height:28px;border:1px solid #ccc;border-radius:4px;padding:0 6px;font-size:13px;'></div>";
    },
    set_value: function(node, value, task, sns) {
        node.querySelector("input").value = value || '';
    },
...
gantt.form_blocks["template"] = {
    render: function(sns) {
        return "<div class='gantt_cal_ltext'><input type='date' id='cal_" + sns.name + "' style='width:130px;height:28px;border:1px solid #ccc;border-radius:4px;padding:0 6px;font-size:13px;'></div>";
    },
    set_value: function(node, value, task, sns) {
...
gantt.form_blocks["operation_progress_lb"] = {
    render: function(sns) {
        return `<div class='gantt_cal_ltext' style='display:flex;align-items:center;gap:6px;'>
            <input type='number' min='0' max='100' step='1' placeholder='0〜100' style='width:80px;height:28px;border:1px solid #ccc;border-radius:4px;padding:0 8px;font-size:13px;text-align:center;'>
            <span style='font-size:13px;color:#555;'>%</span>
        </div>`;
    },
    set_value: function(node, value, task, sns) {
        node.querySelector("input").value = _normalizeOperationProgressStatus(task);
    },
    get_value: function(node, task, sns) {
        const v = node.querySelector("input").value;
        if (v === "" || v === null || v === undefined) return "";
...
gantt.form_blocks["date_range"] = {
    render: function(sns) {
        return `<div class='gantt_cal_ltext' style='display:flex;gap:6px;align-items:center;'>
            ...
            <input type='date' id='cal_start_date' style='width:130px;height:28px;border:1px solid #ccc;border-radius:4px;padding:0 6px;font-size:13px;'>
            <span id='cal_end_date_label' style='font-size:12px;white-space:nowrap;color:#555;'>完了予定日</span>
            <input type='date' id='cal_end_date' style='width:130px;height:28px;border:1px solid #ccc;border-radius:4px;padding:0 6px;font-size:13px;'>
        </div>`;
    },
    set_value: function(node, value, task, sns) {
...
function _getDrawingColumns() {
    return _applyStoredDrawingWidths([
        ...
        { name: "status",         label: "進捗",         width: 60, align: "center",
...
function _getTripColumns() {
    return _applyStoredTripWidths([
        ...
        { name: "customer_name",   label: "客先",   width: 100, align: "left", editor: { type: "text", map_to: "customer_name" } },
        { name: "project_details", label: "工事名", width: 100, align: "left", editor: { type: "text", map_to: "project_details" } },
        { name: "text",            label: "タスク", width: 90,  tree: true,      editor: { type: "task_select", map_to: "text" } },
